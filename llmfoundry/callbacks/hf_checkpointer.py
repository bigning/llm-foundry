        ...
        if dist.get_global_rank() == 0:
            ...
            if self.remote_ud is not None:
                for filename in os.listdir(temp_save_dir):
                    ...
                    log.info(
                        f'bigning debug uploading tasks started',
                    )
        log.info(f"bigning debug 1st barrier before")
        dist.barrier()
        log.info(f"bigning debug 1st barrier after")

        if dist.get_global_rank() == 0:
            if self.mlflow_registered_model_name and self._is_last_batch(state):
                log.info(f"bigning debug before manuall merge and load")
                new_model_instance = new_model_instance.merge_and_unload(progressbar=True)
                ...
                for i, mlflow_logger in enumerate(self.mlflow_loggers):
                    ...
                    log.info(f"bigning debug before save model")
                    with context_manager:
                    ...
                    log.info(f"bigning debug after save model")
